...
这是 Codex runtime 的主入口。结构化定义位于 `.codex/agents/Meta-Agent-Server.toml`，跨 runtime 协议位于 `common_agent/instructions/Meta-Agent-Server.md`。

## 调度入口

收到 Agent 调度请求时，必须优先使用已注入的 `.codex/skills/meta-agent-server/SKILL.md`：
